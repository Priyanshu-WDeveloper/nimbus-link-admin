    ...
    <AdminShell title="Dashboard">
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="panel-surface">
          <div className="flex items-center justify-between px-6 pt-5">
            ...
            <button className="flex items-center gap-1 rounded-md border border-border bg-panel-2 px-2.5 py-1 text-xs font-medium text-muted-foreground hover:text-foreground">
              This Month{' '}
            ...
            <div className="h-[240px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <CartesianGrid strokeDasharray="3 6" stroke="#2a3548" vertical={false} />
                    <XAxis dataKey="day" stroke="#6b7a8f" fontSize={11} tickLine={false} axisLine={false} />
                    <YAxis stroke="#6b7a8f" fontSize={11} tickLine={false} axisLine={false} width={30} />
                    <Tooltip contentStyle={{ background: '#1f2a3d', border: '1px solid #2a3548', borderRadius: 8, fontSize: 12 }} />
                    <Area type="monotone" dataKey="v" stroke={BRAND} strokeWidth={2.5} fill="url(#areaFill)" dot={{ r: 3, fill: BRAND }} activeDot={{ r: 5 }} />
                  </AreaChart>
                </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="panel-surface">
          <div className="px-6 pt-5">
            <h2 className="text-base font-semibold text-foreground">Enquiries by Type</h2>
          </div>
              ...
              {pieData.map((d) => {
                ...
                return (
                  <li key={d.name} className="flex items-center gap-2">
                    ...
                    <span className="ml-auto tabular-nums text-muted-foreground">{d.value} ({pct}%)</span>
                  </li>
                );
              })}
            </ul>
      ...
      <div className="mt-6 panel-surface p-5">
        ...
        <div className="flex flex-row flex-wrap gap-3">
          ...
          <a href="#" className="group flex items-center gap-3 rounded-xl border border-border bg-panel-2 p-3 transition hover:border-brand/50">
            <div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-brand/15 text-brand">
              <ExternalLink className="h-4 w-4" />
            </div>
            <span className="text-sm font-semibold text-foreground">View Website</span>
          </a>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="panel-surface">
          <div className="flex items-center justify-between px-6 pt-5">
            <h2 className="text-base font-semibold text-foreground">Recent New Connections</h2>
            <Link to="/new-connection" className="text-xs font-medium text-brand hover:underline">View All</Link>
          </div>
        ...
        <div className="panel-surface flex flex-col">
          <div className="flex items-center justify-between px-6 pt-5">
            <h2 className="text-base font-semibold text-foreground">Recent Contact Messages</h2>
            <Link to="/contact-us" className="text-xs font-medium text-brand hover:underline">View All</Link>
          </div>
            ...
            {messages.map((m) => (
              <li key={m.name}>
                <button className="flex w-full items-start gap-3 rounded-lg px-4 py-3 text-left transition hover:bg-panel-2">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-panel-2 text-sm font-semibold text-brand ring-1 ring-border">{m.initial}</div>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground/80">{m.meta}</p>
                  </div>
                </button>
              </li>
            ))}
      ...
      <div className="mt-6 panel-surface p-5">
        <h2 className="mb-4 text-base font-semibold text-foreground">Website Status</h2>
        <dl className="divide-y divide-border">
          <StatusRow label="Site Status" value={<span className="inline-flex items-center gap-1.5 text-success"><span className="h-2 w-2 rounded-full bg-success shadow-[0_0_8px] shadow-success" /> Live</span>} />
          <StatusRow label="Last Updated" value="28 May, 2026 · 03:15 PM" />
...
function QuickAction({ to, icon: Icon, title, tone }: {
  to: string;
  ...
  return (
    <Link to={to} className="group flex items-center gap-3 rounded-xl border border-border bg-panel-2 p-3 transition hover:border-brand/50">
